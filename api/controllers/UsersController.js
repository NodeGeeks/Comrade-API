...
module.exports = {
                    ...
                    if (match) {
                        ...
                        bcrypt.genSalt(10, function(err, salt) {
                            ...
                            bcrypt.hash(thingToEncrypt, salt, function () {}, function (err, hash) {
                                ...
                                Users.update( {id: user.id}, {accessToken: hash})
                                        ...
                                        Users.create({ comradeUsername: req.body.username, firstName: req.body.firstName, lastName: req.body.lastName, email: req.body.email, password: req.body.password, accessToken: req.body.accessToken, activationToken: req.body.activationToken}).exec(function aftwards(err, user) {
                                            if (user) {
                                                ...
                                                var mailOptions = {
                                                    ...
                                                    to: req.body.email,
                                                    subject: 'Comrade Account Verification',
    ...
    loginSocialAccount: function (req, res) {
        var provider = req.body.provider,
            ...
            accessToken = 'invalidFromLogin',
            bcrypt = require('bcrypt-nodejs'),
            thingToEncrypt = 'comrade' + _.random(598, 78905478) + socialID + _.random(23, 8300000000),
            socialObject, socialUserObject;

        switch (provider){
            case 'facebook':
                socialObject = { facebookID: socialID };
                socialUserObject = { facebookID: socialID, firstName: firstName, lastName: lastName, accessToken: accessToken };
                break;
            case 'google':
                socialObject = { googleID: socialID };
                socialUserObject = { googleID: socialID, firstName: firstName, lastName: lastName, accessToken: accessToken };
                break;
            case 'twitter':
                socialObject = { twitterID: socialID };
                socialUserObject = { twitterID: socialID, firstName: firstName, lastName: lastName, accessToken: accessToken };
                break;
            case 'linkedin':
                socialObject = { linkedInID: socialID };
                socialUserObject = { linkedInID: socialID, firstName: firstName, lastName: lastName, accessToken: accessToken };
                break;
        }
        ...
        bcrypt.genSalt(10, function(err, salt) {
            ...
            bcrypt.hash(thingToEncrypt, salt, function() {} , function(err, hash) {
                ...
                accessToken = hash;
                Users.find().where(socialObject)
                    .then( function (found) {

                        if (found.length >= 1) {
                            Users.update(socialObject, {accessToken: accessToken})
                                .then(function (updated) {
                                    return res.json(updated);
                                })
                                .fail(function(err){
    ...
    linkSocialAccount: function (req, res) {
        ...
        Users.update({id: id, accessToken:token}, socialObject )
            .then(function (updated){
                return res.json(updated);
            })
            .failed(function(err) {
